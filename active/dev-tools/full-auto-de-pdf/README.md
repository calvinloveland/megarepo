...
- Per-page OCR manifests now also label pages with coarse routing hints (`front-matter`, `body`, `back-matter`, `body-low-quality`) plus a simple quality tier, and `benchmark-corpus` / `benchmark-streaming-corpus` roll those signals up into front-matter and low-quality page summaries.
