...
def test_ocr_pdf_with_surya_engine_path(tmp_path) -> None:
    pdf_path = tmp_path / "book.pdf"
    output_path = tmp_path / "out.txt"
    work_dir = tmp_path / "work"
    pdf_path.write_bytes(b"pdf")
    ...
    def _run(command: list[str], _capture_output: bool) -> str:
        if command[0] == "pdftoppm":
            ...
            (pages_dir / "page-1.png").write_bytes(b"x")
            return ""
        raise AssertionError("tesseract should not be called for surya engine")

    def _factory(_language: str):
        def _reader(image_path: Path) -> str:
            seen_images.append(image_path.name)
            return "Surya OCR text"
    ...
    metrics = ocr_pdf_with_tesseract(
        ...
        ocr_engine="surya",
        run_command=_run,
        surya_reader_factory=_factory,
        which=_which,
    )
    assert metrics["page_count"] == 1
    assert seen_images == ["page-1.png"]
    assert "surya" in output_path.read_text(encoding="utf-8").lower()


def test_build_surya_reader_extracts_text_lines(monkeypatch, tmp_path) -> None:
    ...
    class _Page:
        def __init__(self) -> None:
            self.text_lines = [_Line("line one"), {"text": "line two"}]
    ...
    class _FakeRecognitionPredictor:
        def __init__(self, _foundation):  # noqa: ANN001
            pass

        def __call__(self, _images, det_predictor=None, languages=None):  # noqa: ANN001
    ...
    detection_mod.DetectionPredictor = _FakeDetectionPredictor
    foundation_mod.FoundationPredictor = _FakeFoundationPredictor
    recognition_mod.RecognitionPredictor = _FakeRecognitionPredictor
    surya_pkg.detection = detection_mod
    surya_pkg.foundation = foundation_mod
    surya_pkg.recognition = recognition_mod
    monkeypatch.setitem(sys.modules, "surya", surya_pkg)
    monkeypatch.setitem(sys.modules, "surya.detection", detection_mod)
    monkeypatch.setitem(sys.modules, "surya.foundation", foundation_mod)
    monkeypatch.setitem(sys.modules, "surya.recognition", recognition_mod)

    image_path = tmp_path / "page.png"
    Image.new("L", (1, 1), color=255).save(image_path)
    reader = _build_surya_reader("eng")
    text = reader(image_path)
    assert "line one" in text
    assert "line two" in text


def test_build_surya_reader_without_languages_argument(monkeypatch, tmp_path) -> None:
    from PIL import Image
    ...
    class _FakeRecognitionPredictor:
        ...
        def __call__(self, _images, det_predictor=None):  # noqa: ANN001
            assert det_predictor is not None
            return [{"text_lines": [{"text": "dict one"}, {"text": "dict two"}]}]
    ...
    reader = _build_surya_reader("eng")
    text = reader(image_path)
    assert "dict one" in text
    assert "dict two" in text
