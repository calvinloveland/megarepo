...
class AdversarialCorpusTests(unittest.TestCase):
    """Scanner evaluation against confusable card pairs under degradation.

    Uses cards with the same Pokémon name but different printings (e.g.
    104 different Pikachu cards) combined with degradations:
    JPEG compression (quality 5-20), heavy glare, motion blur, low-light,
    desaturation, blue cast, tilt, and occlusion.

    The corpus is generated by scripts/build_adversarial_corpus.py.

    In addition to the three classic aggregate-level tests (moderate,
    hard, extreme), per-degradation tests are dynamically created by
    load_tests() at module level.  Each degradation and combination
    gets its own test so you can isolate exactly which effects cause
    the most accuracy loss.
    """
    ...
    # Human-readable labels for each degradation (mirrors build script).
    DEG_LABELS: dict[str, str] = {
        ...
        "glare_band":      "Glare + band",
        "motion_blur":     "Motion blur",
        "low_light_desat": "Low-light + desat",
        "blue_cast_soft":  "Blue cast + soft",
        "tilted_occluded": "Tilt + occlusion",
    ...
    @staticmethod
    def _apply_degradation_to_image(
        img: Image.Image, anchor_id: str, deg_name: str
    ) -> Image.Image:
        """Apply a single named degradation to *img* (RGB in, RGB out)."""
        ...
        if deg_name in deg_map:
            ...
            slot = {"visibility": vis, "tilt_degrees": tilt, "render_effects": eff}
            rng = random.Random(f"{anchor_id}-{deg_name}")
            return _apply_card_transform(
                img.convert("RGBA"), slot, rng
            ).convert("RGB")
        return img

    @staticmethod
    def _degradation_key(deg_list: list[str]) -> str:
        """Stable sort-join key for a degradation combination."""
        return "+".join(sorted(deg_list))

    @staticmethod
    def _degradation_label(deg_key: str) -> str:
        """Human-readable label for a degradation key."""
        labels = [
            AdversarialCorpusTests.DEG_LABELS.get(d, d)
            for d in deg_key.split("+")
        ]
        return " + ".join(labels)

    @classmethod
    def _load_corpus(cls) -> dict[str, Any]:
        """Load the corpus JSON, normalising old and new formats."""
        import json as jsonlib
        raw = jsonlib.loads(cls.ADVERSARIAL_CORPUS.read_text())
        # New format: {"cases": {...}, "by_degradation": {...}}
        if "cases" in raw:
            return {
                "levels": raw["cases"],
                "by_degradation": dict(raw.get("by_degradation", {})),
            }
        # Old format: top-level keys are level names.
        levels: dict[str, list[dict[str, Any]]] = {}
        by_deg: dict[str, list[dict[str, Any]]] = {}
        for key, cases in raw.items():
            if isinstance(cases, list):
                levels[key] = cases
                for case in cases:
                    dk = cls._degradation_key(case.get("degradations", []))
                    by_deg.setdefault(dk, []).append(case)
        return {"levels": levels, "by_degradation": by_deg}
    ...
    def _run_cases(
        ...
        """Run the CLIP pipeline on *cases*.

        Returns (correct, total, accuracy).  Skips cases whose reference
        image is missing.
        """
        import pokemon_binder_scanner.scanner as s

        s.load_clip_index(str(cls.CLIP_INDEX))
        ...
        for case in cases:
            ...
            img_path = cls.REF_DIR / f"{anchor_id}.png"
            ...
            for deg_name in case["degradations"]:
                degraded = cls._apply_degradation_to_image(
                    degraded, anchor_id, deg_name
                )

            emb = s._clip_embed_slots([degraded])
            indices = s._FAISS_INDEX.search(emb, 40)[1]
            total += 1
    ...
    # ------------------------------------------------------------------
    # Detailed breakdown (subtests per degradation combo)
    # ------------------------------------------------------------------

    def test_breakdown_by_degradation(self) -> None:
        """Run every degradation combination and report per-group accuracy.

        This test uses subTest for each combination so failures are
        reported with the specific degradation key visible in output.
        There is no hard threshold here — the purpose is visibility,
        not gating.  The aggregate-level tests enforce the gates.
        """
        corpus = self._load_corpus()
        by_deg = corpus["by_degradation"]
        if not by_deg:
            self.skipTest("Corpus has no by_degradation grouping (old format).")

        results: list[tuple[str, str, int, int, float]] = []

        for deg_key in sorted(by_deg.keys()):
            cases = by_deg[deg_key]
            if not cases:
                continue
            correct, total, accuracy = self._run_cases(cases)
            label = self._degradation_label(deg_key)
            results.append((deg_key, label, correct, total, accuracy))

            with self.subTest(degradation=deg_key):
                pass  # results collected for summary below

        # Print a human-readable summary table.
        self._print_breakdown_table(results)

    @staticmethod
    def _print_breakdown_table(
        results: list[tuple[str, str, int, int, float]]
    ) -> None:
        """Print a formatted accuracy breakdown table to stdout."""
        print("\n" + "=" * 78)
        print("  Adversarial accuracy breakdown by degradation")
        print("=" * 78)
        print(f"  {'Degradation':<38s} {'Cases':>6s} {'Correct':>8s} {'Acc':>7s}")
        print("  " + "-" * 62)

        # Sort worst-to-best so problem areas are at the top.
        for deg_key, label, correct, total, accuracy in sorted(
            results, key=lambda r: r[4]
        ):
            bar = _accuracy_bar(accuracy)
            print(
                f"  {label:<38s} {total:>6d} {correct:>8d} "
                f"{accuracy:>6.1%}  {bar}"
            )

        if results:
            overall_correct = sum(r[2] for r in results)
            overall_total = sum(r[3] for r in results)
            overall = overall_correct / overall_total if overall_total else 0.0
            print("  " + "-" * 62)
            print(
                f"  {'OVERALL':<38s} {overall_total:>6d} {overall_correct:>8d} "
                f"{overall:>6.1%}"
            )
        print("=" * 78 + "\n")
...
# -----------------------------------------------------------------------
# Dynamically create one test method per degradation combination so each
# appears as a separate test in the runner output.
#
# Methods are injected directly on AdversarialCorpusTests at module load
# time.  Each method reads the corpus JSON and skips if its degradation
# key is missing, so the test file remains importable even when the
# corpus hasn't been built yet.
# -----------------------------------------------------------------------


def _accuracy_bar(accuracy: float, width: int = 24) -> str:
    """Draw a simple ASCII bar for the accuracy breakdown table."""
    filled = int(round(accuracy * width))
    return "█" * filled + "░" * (width - filled)


def _discover_degradation_keys() -> list[str]:
    """Read the corpus and return the sorted list of degradation keys."""
    corpus_path = Path(
        "/data/home/calvin/pokemon-binder-scanner/adversarial_corpus.json"
    )
    ...
    try:
        ...
        return []
    if "by_degradation" in raw:
        return sorted(dict(raw["by_degradation"]).keys())
    # Old format: derive keys from level cases.
    by_deg: dict[str, list[dict[str, Any]]] = {}
    for cases in raw.values():
        if isinstance(cases, list):
            for case in cases:
                dk = AdversarialCorpusTests._degradation_key(
                    case.get("degradations", [])
                )
                by_deg.setdefault(dk, []).append(case)
    return sorted(by_deg.keys())


def _make_per_degradation_test(deg_key: str):
    """Factory: return a test function for a specific degradation key.

    The returned function gracefully skips if the corpus or its key are
    unavailable, so the module is always importable.
    """

    def test(self: AdversarialCorpusTests) -> None:
        if not self.ADVERSARIAL_CORPUS.exists():
            self.skipTest("Adversarial corpus not built.")
        if not (self.CLIP_INDEX / "clip.index").exists():
            self.skipTest("CLIP index not built.")
        corpus = self._load_corpus()
        by_deg = corpus.get("by_degradation", {})
        cases = by_deg.get(deg_key, [])
        if not cases:
            self.skipTest(f"No cases for degradation '{deg_key}'")
        correct, total, accuracy = self._run_cases(cases)
        label = self._degradation_label(deg_key)
        # No hard threshold — the purpose is visibility.
        # The aggregate tests enforce the gates.
        _ = (correct, total, accuracy, label)

    return test


# Inject per-degradation test methods into the class at import time.
for _deg_key in _discover_degradation_keys():
    _safe_name = (
        "test_degradation_" + _deg_key.replace("+", "_").replace("-", "_")
    )
    _method = _make_per_degradation_test(_deg_key)
    _method.__name__ = _safe_name
    _method.__qualname__ = f"AdversarialCorpusTests.{_safe_name}"
    _label = AdversarialCorpusTests._degradation_label(_deg_key)
    _method.__doc__ = f"Degradation: {_label}"
    setattr(AdversarialCorpusTests, _safe_name, _method)
