...
- Pages 35–38 add deliberately scattered random layouts that do not fit the original template families. The scanner now falls back to a picture-only irregular-layout detector for low-confidence pages, which recovers several scattered cards but still misses some low-signal slots.
