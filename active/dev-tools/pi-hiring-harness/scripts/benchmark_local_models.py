#!/usr/bin/env python3
"""Benchmark local Ollama models on the Pig Latin tiny-model task.

Uses:
- CEO: github-copilot/gpt-5.4
- Reviewer: github-copilot/gpt-5.4
- Candidate hires: local Ollama models discovered from `pi --list-models ollama`

The benchmark is resumable and writes one isolated workspace per local model.
"""
...
DEFAULT_ROOT_PREFIX = "pi-local-model-benchmark-"
CEO_MODEL = "github-copilot/gpt-5.4"
REVIEWER_NAME = "reviewer-gpt54"
REVIEWER_MODEL = "github-copilot/gpt-5.4"
PER_MODEL_TIMEOUT_SECONDS = 3600

TASK_OBJECTIVE = (
    "Create and actually train a tiny local Pig Latin model using only Python stdlib and numpy. "
    "Use a deliberately simple architecture that can learn the transformation robustly on CPU, GPU, or NPU via the fastest local runtime available."
)
...
WORKER_PROMPT_TEMPLATE = textwrap.dedent(
    ...
    ---
    name: {name}
    description: Local Ollama candidate backed by {model}
    role: implementer
    model: ollama/{model}
...
def discover_local_models() -> list[str]:
    proc = subprocess.run(["pi", "--list-models", "ollama"], capture_output=True, text=True, check=True)
    stream = proc.stdout if proc.stdout.strip() else proc.stderr
    models = []
    for line in stream.splitlines():
        parts = line.split()
        if len(parts) >= 2 and parts[0] == "ollama" and parts[1] != "model":
...
def detect_local_runtime() -> dict[str, Any]:
    ...
    return {
        "ollamaPs": proc.stdout,
        "note": "Ollama chooses the fastest available backend it can access. On this machine it reported CPU-only inference in the current server log.",
    }
...
def write_workspace(root: Path, model: str) -> tuple[Path, str]:
    ...
    (workspace / "README.md").write_text("# Local model benchmark workspace\n", encoding="utf-8")
...
def render_html(root: Path, results: list[dict[str, Any]], runtime_info: dict[str, Any]) -> Path:
...
<head>
  <meta charset='utf-8'>
  <title>Local Model Benchmark</title>
  <style>
    ...
    a {{ color: #93c5fd; }}
    pre {{ background: #111827; border: 1px solid #334155; padding: 12px; white-space: pre-wrap; }}
  </style>
</head>
<body>
  <h1>Local Ollama Model Benchmark</h1>
  <p>CEO: {CEO_MODEL} · Reviewer: {REVIEWER_MODEL}</p>
  <p>Root: {root}</p>
  <p>{runtime_info.get('note','')}</p>
  <pre>{runtime_info.get('ollamaPs','')}</pre>
  <table>
    <thead>
      <tr>
        <th>Rank</th>
...
def main() -> None:
    ...
    report = render_html(root, results, runtime_info)
