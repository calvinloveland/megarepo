# Web Applications

Standalone web applications for various use cases.

## Applications

### Parameter Exploration & Experimentation
- [**parambulator**](parambulator/README.md) - Web application for interactive parameter exploration and visualization
  - [Deployment Guide](parambulator/DEPLOYMENT.md)
  - Security Documentation:
    - [Security Review](parambulator/SECURITY_REVIEW.md)
    - [Security Summary](parambulator/SECURITY_SUMMARY.md)
    - [Security Hardening Plan](parambulator/SECURITY_HARDENING_PLAN.md)
    - [Security Quick Fix](parambulator/SECURITY_QUICK_FIX.md)

## Related Projects

For web development tools, see:
- [cli-to-web](../dev-tools/cli-to-web/README.md) - Framework for converting CLI tools to web interfaces
- [browser-error-logger](../dev-tools/browser-error-logger/README.md) - Browser error logging utility

## Related Documentation

- [Active Projects Overview](../README.md) - All active projects
- [Development Philosophy](../../PHILOSOPHY.md) - Core development principles
- [Repository Root](../../README.md) - Complete megarepo structure
