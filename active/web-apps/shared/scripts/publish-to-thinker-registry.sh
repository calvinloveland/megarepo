...
DOCKERFILE_RELATIVE_PATH="${DOCKERFILE_RELATIVE_PATH:-$(basename "${APP_DIR}")/Dockerfile}"
...
create_remote_build_dir() {
  REMOTE_BUILD_DIR="$(ssh "${THINKER_HOST}" 'mktemp -d /tmp/web-app-build.XXXXXX')"
}

stream_source_to_remote() {
  tar "${TAR_EXCLUDES[@]}" -C "${BUILD_CONTEXT}" -cf - . | ssh "${THINKER_HOST}" "tar -xf - -C '${REMOTE_BUILD_DIR}'"
}

echo "==> Ensuring private registry is running on ${THINKER_HOST}"
ensure_remote_registry

echo "==> Streaming build context to ${THINKER_HOST}"
create_remote_build_dir
stream_source_to_remote

echo "==> Building and pushing ${REMOTE_IMAGE_TAGGED} and ${REMOTE_IMAGE_LATEST}"
remote_bash "
  build_docker() {
  ...
  build_docker build \
    -f '${REMOTE_BUILD_DIR}/${DOCKERFILE_RELATIVE_PATH}' \
    -t '${REMOTE_IMAGE_TAGGED}' \
    -t '${REMOTE_IMAGE_LATEST}' \
    '${REMOTE_BUILD_DIR}'
  build_docker push '${REMOTE_IMAGE_TAGGED}' >/tmp/${IMAGE_REPOSITORY}-push-tagged.log
  build_docker push '${REMOTE_IMAGE_LATEST}' >/tmp/${IMAGE_REPOSITORY}-push-latest.log
  tail -n 3 /tmp/${IMAGE_REPOSITORY}-push-tagged.log
  tail -n 3 /tmp/${IMAGE_REPOSITORY}-push-latest.log
"
