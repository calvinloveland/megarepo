...
test("mix prompts use rich example lists", async () => {
  ...
  expect(
    countExampleLines(source, "const mixTagExamples"),
  ).toBeGreaterThanOrEqual(8);
  expect(
    countExampleLines(source, "const mixDensityExamples"),
  ).toBeGreaterThanOrEqual(8);
  expect(
    countExampleLines(source, "const mixColorExamples"),
  ).toBeGreaterThanOrEqual(8);
  expect(
    countExampleLines(source, "const mixDescriptionExamples"),
  ).toBeGreaterThanOrEqual(8);
});
