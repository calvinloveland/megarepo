...
test("paint persists after simulation runs and additional painting", async ({
  page,
}, testInfo) => {
  ...
  try {
    ...
    await page.evaluate(() =>
      (window as any).__paintGridPoints?.([{ x: 75, y: 80 }]),
    );
    // wait for grid_set to be processed and UI to render
    await page.waitForFunction(
      () => (window as any).__lastGridSample !== undefined,
      { timeout: 2000 },
    );
    ...
    await page.evaluate(() =>
      (window as any).__paintGridPoints?.([{ x: 80, y: 80 }]),
    );
