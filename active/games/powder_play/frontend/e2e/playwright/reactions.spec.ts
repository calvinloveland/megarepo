...
test("salt reacts with water to form saltwater", async ({ page }, testInfo) => {
...
test("saltwater reacts with fire to form steam and salt", async ({
  page,
}, testInfo) => {
  const logger = createFailureLogger(testInfo, page);
  let failed = false;
