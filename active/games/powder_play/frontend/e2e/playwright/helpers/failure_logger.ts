...
export function createFailureLogger(testInfo: TestInfo, page?: Page): FailureLogger {
  const entries: string[] = [];

  if (page) {
    page.on('console', msg => entries.push(`PAGE LOG: ${msg.text()}`));
    page.on('pageerror', err => entries.push(`PAGE ERROR: ${err.message}`));
    page.on('requestfailed', req => entries.push(`REQUEST FAILED: ${req.url()} - ${req.failure()?.errorText}`));
  }
