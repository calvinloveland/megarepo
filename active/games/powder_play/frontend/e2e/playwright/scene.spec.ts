...
test("demo scene: paint sand and it falls down", async ({ page }, testInfo) => {
  ...
  try {
    ...
    const sceneResp = await page.request.get(
      new URL("/demo/simple_scene.json", page.url()).toString(),
    );
    ...
    const hasInit = await page.evaluate(() => ({
      has: !!(window as any).__initWorkerWithMaterial,
      type: typeof (window as any).__initWorkerWithMaterial,
    }));
