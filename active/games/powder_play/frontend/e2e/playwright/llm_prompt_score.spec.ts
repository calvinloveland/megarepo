  ...
  try {
    ...
    for (const promptSet of promptSets) {
      for (const [a, b] of mixes) {
        ...
        const candidate =
          typeof nameObj?.name === "string" && nameObj.name.trim()
            ? nameObj.name.trim()
            : "TestMix";

        let materialObj: any = null;
        try {
          const materialText = await postLLM(
            promptSet.materialPrompt(a, b, candidate),
          );
    ...
    for (const row of rows) {
      logger.log(
        `${row.prompt} ${row.pair} name=${row.nameValid ? "ok" : "bad"} material=${row.materialValid ? "ok" : "bad"} score=${row.total}/3`,
      );
    }
