...
test("hot-add material is discovered and auto-loaded", async ({
  page,
}, testInfo) => {
  ...
  try {
    ...
    const matPath = path.resolve(
      __dirname,
      "../../../materials/hot_added.json",
    );
    const mat = {
      type: "material",
      name: "HotAdded",
    ...
    const materialLoaded = new Promise<string>((resolve) => {
      page.exposeFunction("onMaterialLoaded", (name: string) => {
        logger.log("onMaterialLoaded called with", name);
        resolve(name);
      });
    });
    ...
    execSync("npm run materials:sync", {
    ...
    await page.waitForFunction(
      async () => {
        try {
          ...
          return (
            j.materials &&
            j.materials.some((m: any) => m.file === "hot_added.json")
          );
        } catch (e) {
          return false;
        }
    ...
    const materialRow = page
      .locator("#materials-list > div", { hasText: "HotAdded" })
      .first();
    ...
    const loadedName = (await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("timed out waiting for material load")),
          10000,
        ),
      ),
    ])) as string;
