export function mountMaterialBrowser(root: HTMLElement) {
  ...
  const discoveredSection = container.querySelector(
    "#discovered-section",
  ) as HTMLElement;
  const discoveredList = container.querySelector(
    "#discovered-list",
  ) as HTMLElement;
  ...
  async function fetchIndex() {
    try {
      ...
      if (!r.ok) {
        listEl.textContent = "(no materials)";
        return [];
      }
      const j = await r.json();
      return j.materials || [];
    } catch (e) {
      listEl.textContent = "(materials unavailable)";
      return [];
    }
  }
  ...
  async function refresh() {
    ...
    const visible = mats.filter((m: any) => starterNames.has(m?.name));
    const signature = `starter:${visible.map((m: any) => m?.file).join("|")}`;
  ...
  function selectRowByName(name: string) {
    const rows = Array.from(
      listEl.querySelectorAll(".materials-row"),
    ) as HTMLElement[];
    for (const r of rows) {
    ...
    const drows = Array.from(
      discoveredList.querySelectorAll(".materials-row"),
    ) as HTMLElement[];
  ...
  async function loadMaterial(file: string) {
    try {
      ...
      if ((window as any).__initWorkerWithMaterial) {
        console.log(
          "[material_browser] initWorkerWithMaterial ->",
          mat.name,
          file,
        );
      ...
      console.warn("load material failed", e);
    }
  }
  ...
  async function ensureReactionMaterials(mat: any) {
    try {
      ...
      const idxResp = await fetch("/materials/index.json", {
        cache: "no-store",
      });
